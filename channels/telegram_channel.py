...
class TelegramChannel(BaseChannel):
    ...
    async def start(self):
        if not self.token:
            print("⚠️ [Telegram] TELEGRAM_BOT_TOKEN manquant dans le .env, le canal Telegram est désactivé.")
            return

        print("🚀 [Telegram] Chargement des modèles STT et TTS...")
        ...
        print("✅ [Telegram] Bot démarré avec succès !")
    ...
    async def _start_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text("🔧 Système : Le bot ARIA est lancé et prêt à interagir.")
    ...
    async def _handle_voice(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        chat_id = str(update.effective_chat.id)
        print("\n[Telegram Voice] Téléchargement et transcription du vocal en cours...")
        ...
        try:
            ...
            await update.message.reply_text("🔧 Système : Erreur interne lors de la transcription.")
            os.remove(ogg_path)
            return
        ...
        if not user_text.strip():
            await update.message.reply_text("🔧 Système : La transcription audio était vide (aucun son détecté).")
            return

        print(f"[Telegram Voice] Vous: {user_text}")
    ...
    async def send_async(self, message: MessageContext):
        ...
        if is_voice and self.voice:
            if not response_text.strip():
                await self.app.bot.send_message(chat_id=chat_id, text="🔧 Système : Aucune réponse textuelle n'a pu être générée par l'IA.")
                return
            ...
            try:
                ...
                print(f"[Telegram] Erreur TTS: {e}")
                await self.app.bot.send_message(chat_id=chat_id, text=f"🔧 Système : Échec du rendu vocal. Voici la réponse texte :\n\n{response_text}")
            finally:
            ...
            if response_text.strip():
                ...
                print("[Telegram] Aucune réponse générée (texte vide).")
